...
class MNIST_Frames(torchvision.datasets.MNIST):
    def __init__(self, nt, train: bool, root: str = "./", download: bool = True,
                 noise_type: str = None, noise_intensities=None):
    ...
    def __getitem__(self, index):
        ...
        if self.noise_type is not None:
            # Pick a random item from list of possible noise intensities and apply to image
            noise_intensity = random.choice(self.noise_intensities)
            img = torch.FloatTensor(add_noise(img, noise_type=self.noise_type, noise_intensity=noise_intensity))
